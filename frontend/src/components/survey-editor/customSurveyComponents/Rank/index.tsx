...
export const RankView = ({
  ...
  const [sorting, setSorting] = useState(
    JSON.parse(JSON.stringify(data.sorting || {}))
  );
  const limit = data.limit || 10;
  const [binList, setBinList] = useState<SortingBinSelected[]>(
    data.binList || []
  );
  ...
  const fillEmpty = (rankList: RankCard[], limit: number) => {
    return [
      ...rankList,
      ...Array(limit - rankList.length).fill({
        id: "",
        title: "",
        isRanked: false,
      }),
      ...
      page.elements?.forEach((element) => {
        if (element.id === data.id) {
          ...
          if (targetPage) {
            ...
            const targetElement = targetPage.elements?.find(
              (element) => element.type === "ranksurvey"
            );
            if (targetElement) {
              updateData(
                targetElement.id,
                { ...targetElement.data, rankList },
                false
              );
            }
            ...
            <div key={`bin_` + bin.id} className="sorting-bin-outter">
              ...
              <div className="sorting-bin-box">
                {binSorting.map((card: RankCard) => {
                  ...
                  return (
                    <Popover content="Rank It">
                      <span
                        onClick={() => handleRankCard(card, bin.id)}
                        className="sorting-card-item-container"
                        key={card.id}
                      >
                        <span className="sorting-card-item-title">
                          {card.title}
                        </span>
                        <Button
                          type="ghost"
  ...
  return (
    <div className="rank-panel">
      <div className="rank-bin-panel">{renderBins()}</div>
...
export const RankConfig = ({ updateData }: RankConfigProps) => {
  ...
  const data: RankData = querySelectedData(surveyJson, elementId) as RankData;
    ...
    for (let i = 0; i < surveyJson.pages.length; i++) {
      if (i > 0 && surveyJson.pages[i].id === selectedPageId) {
        if (surveyJson.pages[i - 1] && surveyJson.pages[i - 1].elements) {
          const prevElementsList: SurveyComponentData[] =
            surveyJson.pages[i - 1].elements || [];
          prevElementsList.forEach((element) => {
            if (element.type === "sortcard") {
              const sortBinList = (
                Array.from(element.binList || []) as SortingBinSelected[]
              )
                .filter((item) => {
              ...
              newBinList.sort(
                (a: SortingBinSelected, b: SortingBinSelected) =>
    ...
    },
    {
      ...
      render: (text: string, record: SortingBinSelected) => (
        <Checkbox
          ...
          onChange={() => onSelectedBin(record.id)}
        >
          Selected
        </Checkbox>
      ),
